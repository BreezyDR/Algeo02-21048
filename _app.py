...
if __name__ == '__main__':
    ...
    mean = Matrix(np.mean([k.buffer for k in images], axis=0))
    imagesDiff = [images[i]-mean for i in range(len(images))]
    # # c' = a.at
    a = Matrix([i.buffer.reshape(-1) for i in imagesDiff])
    
    # in our context atrnsposed and a is reversed
    # print(np.array([[1, 2], [3, 4]]).transpose().shape)
    # print(a.getMatrix().shape, a.getTransposedMatrix().shape)
    covariaccent : np.ndarray = a.getMatrix() @ a.getTransposedMatrix()

    

    eigenvalue, eigenvector = np.linalg.eig(covariaccent)
    
    print('EIGENNNN\n\n', eigenvalue.shape, eigenvector.shape, "sadea")

    print(eigenvector.shape, eigenvalue.shape)
    evSort = np.argsort(-1*eigenvalue)[:10]

    print(imagesDiff[1].shape)
    # eifes1 = eigenvector @ imagesDiff[1]
    
    evecBig = a.getTransposedMatrix() @ eigenvector
    print(eigenvector.shape, evecBig.shape)

    z = Matrix(evecBig.transpose()[9])
    print(z.shape)
    print(z.assign(z.getUnsquashedMatrix()).shape)
    z.cv2show()

    mean.cv2show('sa')
    imagesDiff[9].cv2show('22')
    
    # newEigenValue, newEigenVector =  np.linalg.eig(a.getMatrix()[evSort])

    # evVS =   np.transpose(a.getMatrix()[evSort]) @ evAS

    # print(evAS, '\n\n\n\n', np.transpose(a.getMatrix()[evSort]).shape, evAS.shape)

    # cv2.imshow('test eF', newEigenVector[4])
